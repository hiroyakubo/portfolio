...
def conver_to_onehot(label):
    onehot_arr = np.zeros((label.shape[0], 2))
...
def make_data(data_dir="data/", csv_list = ["train.csv", "test.csv"]):
    """
    タイタニック号データの前処理スクリプト.
    Args: 
        data_dir:   str
    ...
    for c in csv_list:
        ...
        if c == "train.csv":
            ...
            label_arr = conver_to_onehot(label_arr)
